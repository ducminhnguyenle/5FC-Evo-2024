...
snp_filtered_vcf = "test/cauris.subset.snps.PASS.DP30.PEKT.ann.vcf"

# Reformat from vcf to gds object
snpgdsVCF2GDS(snp_filtered_vcf, "test/cauris.subset.snps.PASS.DP30.PEKT.ann.gds", method = "biallelic.only")
...
set.seed(200)
filtered_snpset = snpgdsLDpruning(snp_subset_filtered_gds, ld.threshold = 0.2, autosome.only = FALSE, remove.monosnp = FALSE, maf = NaN, slide.max.bp = 50000)
str(filtered_snpset)
...
filtered_subset_pca <- snpgdsPCA(snp_subset_filtered_gds, snp.id = filtered_snpset_id, num.thread = 2, autosome.only = FALSE, remove.monosnp = FALSE, maf = NaN)

## Variance proportion (%)
filtered_pc_percent <- filtered_subset_pca$varprop*100
head(round(filtered_pc_percent, 2))

## Make a data.frame
filtered_pca_df <- data.frame(sample.id = filtered_subset_pca$sample.id,
    EV1 = filtered_subset_pca$eigenvect[,1],
    EV2 = filtered_subset_pca$eigenvect[,2],
...
group_code <- scan("/mnt/hdd/dminh/Cauris/SNPs/annotation/new/filtered/group.txt", what=character())
